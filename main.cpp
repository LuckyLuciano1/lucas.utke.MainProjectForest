...
void IslandUnderside(int Island[ISLANDH][ISLANDW]);
...
void IslandCultivation(int Island[ISLANDH][ISLANDW])
{
	...
	//IslandUnderside(Island);//creates island underside
...
void IslandUnderside(int Island[ISLANDH][ISLANDW])
{
	for (int y = 0; y <= ISLANDH; y++){//creation of fadeaway (1)
		for (int x = 0; x <= ISLANDW; x++){

			if ((Island[y][x] == WALL ||
				Island[y][x] == 1 ||
				Island[y][x] == WALL_COR_BOT_LEFT_T1 ||
				Island[y][x] == WALL_COR_BOT_RIGHT_T1) &&
				Island[y + 1][x] == 0)
				Island[y + 1][x] = FADE_1;
		}
	}
	for (int y = 0; y <= ISLANDH; y++){//creation of fadeaway (2)
		for (int x = 0; x <= ISLANDW; x++){
			if (Island[y][x] == FADE_1){
				for (int a = 1; a < (4); a++){ //rand() % 4 + 2
					Island[y + a][x] = FADE_2;
				}
			}
		}
	...
	for (int y = 0; y <= ISLANDH; y++){//creation of fadeaway (3)
		for (int x = 0; x <= ISLANDW; x++){


			if (Island[y][x] == WALL_COR_RIGHT && Island[y + 1][x] == FADE_2)
				Island[y + 1][x] = WALL_COR_RIGHT;

			if (Island[y][x] == WALL_COR_LEFT && Island[y + 1][x] == FADE_2)
				Island[y + 1][x] = WALL_COR_LEFT;

			if (Island[y][x] == WALL_COR_LEFT && Island[y + 1][x] == 0)
				Island[y + 1][x] = WALL_COR_BOT_LEFT_T0;

			if (Island[y][x] == WALL_COR_RIGHT && Island[y + 1][x] == 0)
				Island[y + 1][x] = WALL_COR_BOT_RIGHT_T0;
		}
	}

	for (int y = 0; y <= ISLANDH; y++){//creation of fadeaway (4)
		for (int x = 0; x <= ISLANDW; x++){
			if (Island[y][x] == FADE_2 && Island[y + 1][x] == 0)
				Island[y][x] = FADE_END;
		}
	}
	...
	for (int y = 0; y <= ISLANDH; y++){//creation of fadeaway (5)
		for (int x = 0; x <= ISLANDW; x++){
			if (Island[y][x] == WALL_COR_TOP_LEFT_T1){
				a = 1;
				while (Island[y + a][x + 1] != FADE_END){
					Island[y + a][x] = WALL_COR_LEFT;
					a++;
				}
				Island[y + a][x] = WALL_COR_BOT_LEFT_T0;
				a = 1;
			}

			if (Island[y][x] == WALL_COR_TOP_RIGHT_T1){
				a = 1;
				while (Island[y + a][x - 1] != FADE_END){
					Island[y + a][x] = WALL_COR_RIGHT;
					a++;
				}
				Island[y + a][x] = WALL_COR_BOT_RIGHT_T0;
				a = 1;
			}
		}
	}
